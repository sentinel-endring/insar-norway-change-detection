"""
InSAR Time Series Visualization Module
...
class InsarVisualizer:
    ...
    def _find_closest_points_gpu(self, 
                               points_df: pd.DataFrame,
                               target_coords: tuple,
                               max_distance: float = 0.5) -> pd.DataFrame:
        """Find points closest to target coordinates using GPU acceleration."""
        # Convert coordinates to GPU arrays
        coords = cp.array(points_df[['northing', 'easting']].values)
        target = cp.array([float(target_coords[0]), float(target_coords[1])]).reshape(1, 2)
        
        # Calculate distances
        distances = cp.sqrt(cp.sum((coords - target) ** 2, axis=1))
        
        # Find points within max distance
        valid_points = cp.where(distances <= max_distance)[0].get()
        
        if len(valid_points) > 0:
            closest_points = points_df.iloc[valid_points].copy()
            closest_points['distance'] = distances[valid_points].get()
    ...
    def _create_time_series_plot(self,
        ...
        info_text = [
            ...
            #f"Early STD: {point_info['early_std']:.2f}",
            #f"Late STD: {point_info['late_std']:.2f}",
        ...
        for (idx, row), category in zip(points_to_process.iterrows(), categories):
            ...
            if track in self.file_paths:
                ...
                try:
                    ...
                    # Find closest point
                    closest_points = self._find_closest_points_gpu(
                        track_df, coords)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='InSAR Time Series Visualization')
    ...
    try:
        ...
        visualizer = InsarVisualizer(file_paths, args.results)
